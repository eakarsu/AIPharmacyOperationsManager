...
// Gap (Non-AI): No insurance verification automation
// No insurance verification automation
import React, { useState } from 'react';

export default function GapNoInsuranceVerificationAutomationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-insurance-verification-automation/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No insurance verification automation</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No insurance verification automation</p>
