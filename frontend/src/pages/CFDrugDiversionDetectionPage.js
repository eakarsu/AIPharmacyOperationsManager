...
// Custom Feature: Drug diversion detection
// Drug diversion detection: Real-time analysis of prescription patterns (repeats, quantities, scripts per patient); AI flags suspicious activity; auto-escalates to DEA if warranted
import React, { useState } from 'react';

export default function CFDrugDiversionDetectionPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-drug-diversion-detection/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Drug diversion detection</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Drug diversion detection: Real-time analysis of prescription patterns (repeats, quantities, scripts per patient); AI flags suspicious activity; auto-escalates to DEA if warranted</p>
