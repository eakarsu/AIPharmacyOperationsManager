...
// Gap (AI): Patients without '/adherence
// Patients without /adherence-predict (flag likely non-adherence; suggest interventions)
import React, { useState } from 'react';

export default function GapPatientsWithoutAdherencePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-patients-without-adherence/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Patients without '/adherence</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Patients without /adherence-predict (flag likely non-adherence; suggest interventions)</p>
