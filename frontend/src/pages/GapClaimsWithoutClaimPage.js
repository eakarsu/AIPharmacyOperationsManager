...
// Gap (AI): Claims without '/claim
// Claims without /claim-denial-predict (predict if claim will be denied)
import React, { useState } from 'react';

export default function GapClaimsWithoutClaimPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-claims-without-claim/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Claims without '/claim</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Claims without /claim-denial-predict (predict if claim will be denied)</p>
