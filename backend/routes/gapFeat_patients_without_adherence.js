...
// Gap Feature: Patients without \'/adherence
// Patients without /adherence-predict (flag likely non-adherence; suggest interventions)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPharmacyOperationsManager / gap-patients-without-adherence',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Patients without \'/adherence'.\nProject: AIPharmacyOperationsManager.\nContext: Patients without /adherence-predict (flag likely non-adherence; suggest interventions)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['patients-without-adherence', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'patients-without-adherence', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
