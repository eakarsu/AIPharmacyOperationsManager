...
// Gap Feature: Claims without \'/claim
// Claims without /claim-denial-predict (predict if claim will be denied)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPharmacyOperationsManager / gap-claims-without-claim',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Claims without \'/claim'.\nProject: AIPharmacyOperationsManager.\nContext: Claims without /claim-denial-predict (predict if claim will be denied)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['claims-without-claim', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'claims-without-claim', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
