...
// Gap Feature: Controlled without \'/diversion
// Controlled without /diversion-detect (flag suspicious prescribing patterns suggesting drug diversion)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPharmacyOperationsManager / gap-controlled-without-diversion',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Controlled without \'/diversion'.\nProject: AIPharmacyOperationsManager.\nContext: Controlled without /diversion-detect (flag suspicious prescribing patterns suggesting drug diversion)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['controlled-without-diversion', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'controlled-without-diversion', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
