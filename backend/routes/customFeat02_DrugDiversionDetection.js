...
// Real-time analysis of prescription patterns (repeats, quantities, scripts per patient); AI flags suspicious activity; auto-escalates to DEA if warranted
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPharmacyOperationsManager / cf-drug-diversion-detection',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_drug_diversion_detection`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-drug-diversion-detection feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Drug diversion detection'.
Context: Real-time analysis of prescription patterns (repeats, quantities, scripts per patient); AI flags suspicious activity; auto-escalates to DEA if warranted
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-drug-diversion-detection/run error:', err);
