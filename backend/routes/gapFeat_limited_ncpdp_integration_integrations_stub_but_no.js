...
// Gap Feature: Limited NCPDP integration (integrations stub but no real network adapter)
// Limited NCPDP integration (integrations stub but no real network adapter)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPharmacyOperationsManager / gap-limited-ncpdp-integration-integrations-stub-but-no',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited NCPDP integration (integrations stub but no real network adapter)'.\nProject: AIPharmacyOperationsManager.\nContext: Limited NCPDP integration (integrations stub but no real network adapter)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-ncpdp-integration-integrations-stub-but-no', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-ncpdp-integration-integrations-stub-but-no', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['limited-ncpdp-integration-integrations-stub-but-no']
    );
