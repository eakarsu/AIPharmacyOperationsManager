...
// Continuous review of patient med list; AI identifies deprescribing opportunities, duplicate therapies, cost-saving generics; generates MTM interventions
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPharmacyOperationsManager / cf-medication-therapy-management-mtm',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_medication_therapy_management_mtm`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-medication-therapy-management-mtm feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Medication therapy management (MTM)'.
Context: Continuous review of patient med list; AI identifies deprescribing opportunities, duplicate therapies, cost-saving generics; generates MTM interventions
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-medication-therapy-management-mtm/run error:', err);
