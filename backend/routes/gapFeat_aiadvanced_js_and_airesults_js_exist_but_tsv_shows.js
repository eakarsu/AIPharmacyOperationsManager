...
// Gap Feature: \'aiAdvanced.js\' and \'aiResults.js\' exist but TSV shows 0 endpoints
// aiAdvanced.js and aiResults.js exist but TSV shows 0 endpoints — stubs need wiring
// Project: AIPharmacyOperationsManager
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPharmacyOperationsManager / gap-aiadvanced-js-and-airesults-js-exist-but-tsv-shows',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: '\'aiAdvanced.js\' and \'aiResults.js\' exist but TSV shows 0 endpoints'.\nProject: AIPharmacyOperationsManager.\nContext: aiAdvanced.js and aiResults.js exist but TSV shows 0 endpoints — stubs need wiring\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['aiadvanced-js-and-airesults-js-exist-but-tsv-shows', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'aiadvanced-js-and-airesults-js-exist-but-tsv-shows', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['aiadvanced-js-and-airesults-js-exist-but-tsv-shows']
    );
