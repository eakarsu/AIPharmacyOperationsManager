...
// AI predicts which drugs will need pre-auth; auto-generates and submits requests; tracks approval status; notifies prescriber of denials
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPharmacyOperationsManager / cf-insurance-pre-authorization-automation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_insurance_pre_authorization_automation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-insurance-pre-authorization-automation feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Insurance pre-authorization automation'.
Context: AI predicts which drugs will need pre-auth; auto-generates and submits requests; tracks approval status; notifies prescriber of denials
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-insurance-pre-authorization-automation/run error:', err);
