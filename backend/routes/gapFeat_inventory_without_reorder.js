...
// Gap Feature: Inventory without \'/reorder
// Inventory without /reorder-predict (demand forecasting)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPharmacyOperationsManager / gap-inventory-without-reorder',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Inventory without \'/reorder'.\nProject: AIPharmacyOperationsManager.\nContext: Inventory without /reorder-predict (demand forecasting)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['inventory-without-reorder', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'inventory-without-reorder', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
