...
// Gap Feature: Limited patient counseling tools
// Limited patient counseling tools
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPharmacyOperationsManager / gap-limited-patient-counseling-tools',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited patient counseling tools'.\nProject: AIPharmacyOperationsManager.\nContext: Limited patient counseling tools\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-patient-counseling-tools', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-patient-counseling-tools', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
