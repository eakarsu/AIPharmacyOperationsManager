...
// Gap Feature: MTM module exists but workflow depth unclear
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPharmacyOperationsManager / gap-mtm-module-exists-but-workflow-depth-unclear',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'MTM module exists but workflow depth unclear'.\nProject: AIPharmacyOperationsManager.\nContext: MTM module exists but workflow depth unclear\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['mtm-module-exists-but-workflow-depth-unclear', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'mtm-module-exists-but-workflow-depth-unclear', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
